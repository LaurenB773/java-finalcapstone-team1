<template>
  <main v-if="exercises.length > 0">
    ...
    <div v-for="exercise in exercises" :key="exercise.exerciseId" class="card">
      ...
      <svg
        ...
        class="feather feather-trash trash-icon"
      >
...
export default {
  ...
  mounted() {
    ExerciseService.getExercises().then((response) => {
      ...
      for (const exercise of this.exercises) {
        this.getEquipment(exercise.exerciseId).then((response) => {
          exercise.equipment = response.data;
        });
      }
    });
  },
  ...
  methods: {
    getEquipment(exerciseId) {
      return ExerciseService.getEquipmentForExercise(exerciseId);
    },
